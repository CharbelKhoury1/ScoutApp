...
<head>
  ...
  <style>
    ...
    .view-icon{
      ...
      height: 50px;
      margin-right: 5px;
      background-image: url('../views/images/view-svgrepo-com.svg'); 
    ...
    .download-icon {
      ...
      height: 40px;
      margin-right: 5px;
      background-image: url('../views/images/file-download-svgrepo-com.svg'); 
      background-size: cover;
      filter: grayscale(100%);
...
function displayTransactionRecords() {
  ...
  for (const key in currentData.transactionRecords[0]) {
    if (key !== 'transaction_id') {
      // Exclude the 'transaction_id' column
      const th = document.createElement('th');
      th.textContent = key;
      headerRow.appendChild(th);
    }
  }
  // Add the "Attachment" column header
  ...
  currentData.transactionRecords.forEach(record => {
    ...
    const attachmentCell = document.createElement('td');
      // If attachment exists, create view and download buttons
      const attachmentViewButton = document.createElement('a');
      attachmentViewButton.href = record.attachment; // Set the attachment URL
      attachmentViewButton.target = '_blank';
      attachmentViewButton.classList.add('view-icon');
      attachmentViewButton.dataset.transactionId = record.transaction_id;
      attachmentViewButton.dataset.action = 'view';
      attachmentViewButton.addEventListener('click', handleActionButtonClick);

      const attachmentDownloadButton = document.createElement('a');
      attachmentDownloadButton.href = record.attachment; // Set the attachment URL
      attachmentDownloadButton.target = '_blank';
      attachmentDownloadButton.classList.add('download-icon');
      attachmentDownloadButton.dataset.transactionId = record.transaction_id;
      attachmentDownloadButton.dataset.action = 'download';
      attachmentDownloadButton.addEventListener('click', handleActionButtonClick);
      

      attachmentCell.appendChild(attachmentViewButton);
      attachmentCell.appendChild(attachmentDownloadButton);
      */
  ...
  tableContainer.appendChild(table); // Append the new table to the container
        ...
        function handleActionButtonClick(event) {
            ...
            if (action === 'delete') {
                ...
                // Redirect to the update page with the transaction ID
        ...
        // Initial load - Fetch transaction records

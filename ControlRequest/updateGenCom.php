...
if (isset($_COOKIE['request_id'])) {
  $selectedRequestId = $_COOKIE['request_id'];
    ...
    $query = "UPDATE `requeststatus` SET `date`=SYSDATE(),`statusCode`='1', `flag` = 2 WHERE request_id= $selectedRequestId";
    $result = mysqli_query($conn, $query);
  
    if ($result) {
      header("Location: ../ControlRequest/controlReqGenCom.php?error=Request Approved!");
    } else {
